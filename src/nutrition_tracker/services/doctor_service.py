...
from nutrition_tracker.repositories.correction_repo import iter_correction_files
...
def run_doctor(root: Path) -> dict[str, Any]:
    ...
    for path in iter_correction_files(root):
        try:
            for line_number, correction in iter_jsonl_with_line_numbers(path):
                try:
                    validate_payload(root, "correction-entry.schema.json", correction)
                    if correction["meal_id"] not in meal_ids:
                        errors.append(
                            f"{path}:{line_number}: unknown correction meal_id {correction['meal_id']}"
                        )
                except NutritionTrackerError as exc:
                    errors.append(f"{path}:{line_number}: {exc}")
        except NutritionTrackerError as exc:
