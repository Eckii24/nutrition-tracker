...
def test_daily_summary_uses_stored_meals_without_projection(initialized_root: Path, runner: CliRunner):
    meal_id = add_sample_meal(initialized_root, runner)

    result = runner.invoke(app, ["meal", "list", "2026-05-31", "--path", str(initialized_root), "--json"])
    assert result.exit_code == 0, result.output
