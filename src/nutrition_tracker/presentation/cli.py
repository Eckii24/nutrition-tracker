...
from nutrition_tracker.application.daily import build_daily_summary, list_meals
from nutrition_tracker.application.doctor import run_doctor
from nutrition_tracker.application.goals import derive_and_save_goals
from nutrition_tracker.application.init_project import init_project
from nutrition_tracker.application.meals import add_meal
from nutrition_tracker.application.weekly import build_weekly_summary
from nutrition_tracker.domain.errors import NutritionTrackerError
from nutrition_tracker.infrastructure.project_paths import resolve_root

app = typer.Typer(
    name="nutrition",
    help="Deterministic local backend for file-based nutrition tracking.",
)
...
MealListRootPathOption = Annotated[
    Path,
    typer.Option(
        "--path",
        file_okay=False,
...
DoctorRootPathOption = Annotated[
    Path,
    typer.Option(
        "--path",
        file_okay=False,
...
@app.command(
    "init",
    help="Create the directory layout, schemas, and default settings for a nutrition project.",
)
def init_command(
    path: InitRootPathOption = Path("."),
...
@goals_app.command(
    "derive",
    help="Derive daily macro targets from data/settings.json and write goal snapshots.",
)
def goals_derive_command(
    path: GoalsRootPathOption = Path("."),
    json_output: JsonOption = False,
) -> None:
    try:
...
@meal_app.command(
    "add",
    help="Validate one structured meal payload and append it to the meal event log.",
)
def meal_add_command(
    file: MealFileOption,
    path: MealRootPathOption = Path("."),
    json_output: JsonOption = False,
) -> None:
    try:
...
def meal_list_command(
    ...
    path: MealListRootPathOption = Path("."),
    json_output: JsonOption = False,
) -> None:
    try:
...
@app.command(
    "doctor",
    help="Validate the repo structure and check stored data against the JSON schemas.",
)
def doctor_command(
    path: DoctorRootPathOption = Path("."),
    json_output: JsonOption = False,
) -> None:
    try:
